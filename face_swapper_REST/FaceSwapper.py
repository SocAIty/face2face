        ...
        for attr in dir(face):
            if not attr.startswith("__"):
                try:
                    ...
                    try:
                        if type(face.attr) in [np.float32, np.float64, np.int32, np.int64]:
...
class FaceSwapper:
    def __init__(self,
        ...
        self.face_swapper = insightface.model_zoo.get_model(os.path.join(model_path, inswapper_model_name))
        ...
        self.reference_faces = self.__load_reference_faces_from_folder(reference_faces_folder)
    ...
    def _get_face_analyser(self, model_path: str, providers, det_size=(320, 320)):
        ...
        face_analyser = insightface.app.FaceAnalysis(name="buffalo_l", root=f"{model_path}./checkpoints", providers=providers)
    ...
    def swap_one_image(self, source_image: np.array, target_image: np.array) -> np.array:
        """
        Changes the face(s) of the target image to the face of the source image.
    ...
    def __load_reference_faces_from_folder(folder_path: str) -> dict:
        ...
        for file in files:
            ...
            try:
                saved_faces = np.load(os.path.join(folder_path, file), allow_pickle=True)
                reference_faces[face_name] = [FileWriteableFace.to_face(fa) for fa in saved_faces]
    ...
    def add_reference_face(self, face_name: str, ref_image: np.array, save=False) -> Union[str, np.array]:
        """
    ...
    def swap_from_reference_face(self, face_name: str, target_image: Union[np.array, list]) -> np.array:
        ...
        if face_name not in self.reference_faces:
            raise Exception(f"Reference face {face_name} not found. Please add the reference face first with add_reference_face")
